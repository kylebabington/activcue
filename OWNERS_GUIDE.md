...
- **Phase 1 growth funnel** (canonical names): `landing_page_viewed` → `demo_started` → `demo_completed` → `signup_started` → `signup_completed` → `first_activity_generated` → `checkout_started` → `subscription_started` (shown as “Subscription started” in Admin Growth). Keep client/server allowlists in sync (`src/utils/analytics.js` + `server/lib/productEventNames.js`). `signup_completed` is server-owned on convert-anonymous (do not also fire from the client).
...
- **Growth dashboard** — Admins (`profiles.role = admin`) open `/admin/growth` or `GET /api/admin/growth?range=7d`. Shows a live Launch trial box (claimed / in checkout / remaining / converted to paid from `launch_trial_claims`), the 8-step ad funnel (landing → demo → demo completed → signup started/completed → first activity → checkout → subscription started), step conversion ratios, and UTM breakdown. Grant admin with `node server/scripts/setProfilePrivileges.js --user-id <uuid> --role admin`.
